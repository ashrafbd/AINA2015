...
Development of reliable hardware, ability to sense the physical, flexibility in deployment at scale are the main careers towards future smart environment.
...
Well developed secure mechanisms like WPS, PKM and likewise are inappropriate in WSN. %WPS - WiFi Protected Setup and PKM - Privacy and Key Management for WiMAX
...
An Intrusion Detection System (IDS) does not prevent the break-in, but it can detect  break-in events but can raise alert following some predefined procedures in case of a security breach.
...
We propose an algorithm, which considers a centralised to detect  the intrusion in software update.
...
The information carried by datagram enables the IDS with a centralised view of a network-wide knowledge that is processed and analysed to work out a  `Surprise Score' or `Intrusion Warning Score' (IWS) to indicate a possible intrusion.
...
The following section describes the background and motivation of the study, including how it relates to earlier research that has been performed in the field.
...
Secondly, most of the the OTA update protocols generally do not cater enough cryptographic protection for source authentication, integrity verification and likewise. 
...
However, a malicious intruder can be capable of replicating an authentic image source and advertise new program version which the motes cannot repudiate. 
%\notedme{This needs to be explained. It isn't convincing at the moment IMO.} %%<Ashraf>>>Tried to address
Thirdly, even though,  Deluge incorporates on-chip hardware based encryption, cryptography alone cannot completely prevent attacks. %on WSNs confidentiality, authenticity, availability due to resource limitation.
...
He can utilise the knowledge to modify his code, even can replace the nodes with the illegitimate and malicious ones, which can imitate a legal update process. %\cite{DBLP:journals/corr/abs-1301-3022}.
Resource constraint, use of wireless medium, security-sensitive data, uncontrolled environment, distributed deployment and so on adds to the physical vulnerability of the sensors.
...
In case of failures, reporting an incident and identifying the extent of illegitimate software update is also equally important.
During the software update process, the running OS need to reboot to the new application. 
...
An attacker can mask an illegitimate update and inject his own program image to serve his intended purpose.
...
This can result into complete compromising the operations and functions.
...
Security schemes in WSNs have two main approaches -- using cryptography to secure communication and  employing Intrusion Detection System (IDS) to oversee activities.
...
Designing cryptography based security solutions in the lower layers for wireless sensors are complicated, expansive and often may not be effective.
...
It can be argued, traffic analysis approach is potentially comparable to proposed timing analysis.
...
There is a significant increase in traffic owing to software update process. 
This outs the mechanism off balance.
The other shortcoming of the technique is, certain rare yet critical activity, for example: a tsunami or earthquake, would raise false positive intrusion alarm.
However, the mechanism of statistical treatment on information over a time period motivates us to employ timing analysis approach on a context of OTA update protocol.
...
In the elliptical topology intrusion scenario, maximum IWS reported in the presented data in Figure~\ref{subfig:ellipgraph} was 535. 
