...
The other driving forces behind the WSN are advancement in reliable hardware, flexibility in deployment and so on.
...
MOtes operating within range and sharing same protocols and radio channel form an autonomous network called a WSN.
A WSN does not have routers or gateways or routing tables.
It transports information using a neighbourhood discovery process.

Motes are generally deployed in a large number for long term in uncontrolled environment, which is difficult to manage.
Once deployed, the sensors usually require reprogramming to accommodate bug-fixes, changes in sensing application or scientific requirements or alteration in the environment.
Network code propagation using over the air (OTA) data dissemination protocols provide solution to this problem.

There are several protocols like Trickle, MNP, ReMo, Deluge to name a few, which deal with OTA reprogramming.
While few of these protocols allow updating subset of nodes in the WSN, thereby facilitate preferential treatment of different task at different regions of the same WSN, most of the protocols update the  whole network at one go resulting into all nodes in the WSN running same updated version of the software.
Deluge is an efficient and reliable protocol that allows OTA propagation of program binaries to all nodes in the WSN.
It addresses several network programming issues e.g., deals with program objects that can be larger than the sensor memory, works in environments with high packet loss rate, variable node density and asymmetric links. [CITE 48].
...
Security is an important aspect of WSN development.
Implementation of security techniques in WSN are critical and challenging because of constrained resources in the sensor.
The capability of digital computing devices that can be employed to computationally compromise security of sensors grows at an enormous pace in comparison to that of sensors.
...
Wireless channels are inherently considered less secure.
Secure techniques like WPS, PKM and likewise cannot be employed in WSN because of energy, memory and computing resource constraints. %WPS - WiFi Protected Setup and PKM - Privacy and Key Management for WiMAX
As a result, WSNs are more vulnerable to intrusion attempts and security threats from malicious attackers.
Beside physical attack on sensors, an attacker may also target wireless channels, network visibility, neighbourhood discovery process, even the software update process itself and so on.
...
An IDS does not prevent the break-in, but it can detect  break-in events but can raise alert following some predefined procedures in case of a security breach.
Sometimes, it may be able to report the location where the break-in event has taken place, and even the extent of break-in as well.
...
We propose an algorithm, which is essentially a centralised IDS that prevents the intrusion in software update.
...
AM is a single-hop, unreliable packet which are used to provide synchronous acknowledgements in TinyOS. %%It has a destination address, a type field, and a
...
The information carried by datagram equips the IDS with a centralised view of a network-wide knowledge that is processed and analysed to work out a  `Surprise Score' or `Intrusion Warning Score' (IWS) to indicate a possible intrusion.
...
The following section describes the background and motivation of the study, including how it relates to earlier research that has been done in the field.
...
However, we contend that it is possible for a malicious intruder to replicate an authentic image source and advertise new program version which the motes cannot repudiate. 
%\notedme{This needs to be explained. It isn't convincing at the moment IMO.} %%<Ashraf>>>Tried to address
Even though,  Deluge incorporates on-chip hardware based encryption, Cryptography alone cannot completely prevent attacks. %on WSNs confidentiality, authenticity, availability due to resource limitation.
Techniques of different types of attacks  on WSN have been described by \cite{DBLP:journals/corr/abs-1301-3022}.
Countermeasure  against these attacks are computationally expansive/costly and energy hungry, which implies they cannot practically be employed in resource constrained sensors. 
An attacker can extract valuable information about the network from a physically compromised node.
He can utilise the knowledge to modify his code, even can replace the nodes with the illegitimate and malicious ones. %\cite{DBLP:journals/corr/abs-1301-3022}.
...
During the software update process, the running OS need to reboot to switch to the new application. 
In the process, security  measures on the sensor motes are deactivated or unavailable for a short period of time.
...
An attacker can mask a legitimate update and inject his own program image to serve his intended purpose, which could range from stealing information or logging network events to denial of service. 
%Researchers put lot of effort to study these attacks and to devise the methods to minimize their impact on the network.
Therefore, protecting the update process or in case of failures identifying the incident and extent of illegitimate software update is of great importance.


The other kind of vulnerability of such epidemic natured update protocol is breach at any one point does the job of weakening the whole network for the attacker. 
This can result into complete compromising the operations and functions of the whole WSN.
...
Because  of the absence of the session and presentation layers, applying encoding sockets and end to end encryption is impossible.
Designing crypto based security solutions in the lower layers for wireless sensors are complicated, expansive and often may not be effective.
...
In recent years, researchers have suggested several intrusion or anomaly detection means to secure WSN, beside the other cryptographic approaches.
...
Rule based  techniques rely on monitoring of group of nodes and routing tables for detection~\cite{ISI:000298891500099}, \cite{Chen:2009:NMI:1516241.1516282}, \cite{1424814}, \cite{Strikos_afull}.
...
Specification based  IDS has been proposed that relies on data clustering and uses standard deviation from the average inter-cluster distance~\cite{Chen:2009:NMI:1516241.1516282}, \cite{4085803}.
...
Some anomaly  detection techniques look for anomalies in routing pattern and uses  hidden Markov model to decide on the accuracy of the gathered data~\cite{1290173}, \cite{4024996}.%\notedme{??}
...
An IDS based on game theory that utilises Markov chains for its decision making and monitors only one of the clusters of the network at a time, which leaves the rest of the network unprotected.
The approach optimises the extra transmission or computation needed to defend by taking reactive approach in the game and assuming to be able to always identify the right cluster to defend using Markov chains.
~\cite{1347798}, \cite{Das07preventingdos}, \cite{Reddy:2009:GTA:1607720.1607944}.
%\notedme{why?}
...
The absence of a passive approach to watch over the security breach of the software update leaves an easy door  to intrude upon.
...
While timing analysis technique is non-existent in the WSN, some of the IDSs use traffic analysis or packet inspection  techniques and process them in centralised or distributed fashion.
It can be argued, traffic analysis approach is potentially same as timing analysis we have proposed.
The argument can be quite intense because traffic analysis considers both throughput and latency , and latency essentially about timing.
In response to this, we reply that latency considers the time interval i.e, time delay between a cause and the effect in the system being observed from a holistic view.
In our case, we 
<><><><><>I AM UPDATIING THIS PORTION HERE<><><><><><> aSHRAF
0.	INDICATE WHAT IS THE DIFFERENCE [IN TREATMENT OF THE PROBLEM]
1.  Specifically contrast timing approach against a latency measure within the best matched traffic analysis paper that you’ve found so far.    
2.  they are motivated by the similar technicques in other wireless technologies.
    i NEED TO support MY view HERE,
3.  justify WHATS WRONG WITH  using techniques from other wireless tech,  is not best plan.
4.  other issue is, no IDS specifically addresses software updates
<><><><><><><><><><><><><><><><><><><><><><><><><><><><>
...
This raw data is processed to determine an IWS.
...
\paragraph*{Step 1:} In the environmental set up described above , all motes in the WSN were booted up at same time running one version of a diagnostic application. 
...
\paragraph*{Step 2:} The base station dynamically began the update process and propagated another version of the software (also called image) using Deluge protocol. %, which is also called 
...
\paragraph*{Step 3:} Processed data from 20 simulations, initiated from the sink, were considered for each topology.
...
\item The absolute difference is divided by a function of standard deviation to avoid the possibility of division by zero situation. %This also smooths the data to avoid unrelated variations.\notedme{what's an unrelated variation?}
...
The technique of associating such interpretation has been established identifying Intrusion Warning Zones (IWZ) by using heuristics   and trial and error methods.
...
Figure~\ref{subfig:elliptopo} shows the actual relative positions of the motes with mote ID written at the centre of the tiny circle and IWS measured when an intrusion attempt has been initiated from the mote beneath the circle.
...
Nevertheless, the  established two threshold levels at 10\% and at 30\% that were useful in zone classification using intuitive heuristics.
The IDS suggests different measures for motes in different zones which are discussed bellow.
...
%\notedme(1){zero is just an English word. Why are you typesetting it differently? NB: if you were to typeset it in maths, this is not the correct way to do so: \LaTeX doesn't know that you mean zero to be a word, and thus will instead typeset it as a product of individual variables z, e, r, o, ...}
%% <Ashraf> Addressed 
...
\section{Conclusion}
\label{sec:concl}
%<Ashraf>> I am rewriting this part. ;)
