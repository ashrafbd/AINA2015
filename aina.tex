...
The IDS observes update behaviour effected by the Deluge protocol.
...
It addresses several network programming issues e.g., deals with programme objects that can be larger than the sensor memory, works in environments with high packet loss rate, variable node density and asymmetric links. [CITE 48].
It also ensures quick propagation of programme codes with minimum service interruption, which make it an elegant protocol of choice for  WSN reprogramming to many researches and professionals.
...
An attacker can mask a legitimate update and inject his own programme image to serve his intended purpose, which could range from stealing information or logging network events to denial of service. 
...
Despite the need for augmenting security in WSNs, only few previous studies have considered IDS approach in recent years.
...
%\notedme{What does this mean? Sounds almost exactly like what you're doing, no?}  %<Ashraf>>The technique is Hierarchy based on clusters.They use statistical measures. avoid statistical analysis?
Statistical based anomaly  detection looks for anomalies in routing pattern~\cite{4024996}.
A serious weakness with these methods, however, is that  they use hidden Markov models to focus on the accuracy of the gathered data, rather than the security of the nodes or the links~\cite{1290173}.\notedme{??}
...
An IDS based on game theory that utilises Markov chains for its decision making process can monitor only one of the clusters of the network at a time, which leaves the rest of the network unprotected~\cite{1347798}, \cite{Das07preventingdos}, \cite{Reddy:2009:GTA:1607720.1607944}.\notedme{why?}
...
The WSN OSs do not incorporate an IDS approach due to the reality that deploying such software would cut down on the resources, especially power.
...
However, an IDS does not prevent the break-in, but raises an alert following some predefined procedures in case of a security breach.
...
We propose an algorithm that requires a minor modification of the OTA update protocol to send back a message indicating update timing. %by incorporating Active Message (AM). 
...
I DONT KNOW IF I WANT TO KEEP THIS PARA.\notedme{It doesn't read well enough for me to tell, at the moment, although it looks like evidence is missing...}
...
\notedme{Compare the commented and rewritten sentences here. Terms like `cautiously' are not useful unless the context is clear.}
...
\item The absolute difference is divided by a function of standard deviation to avoid the possibility of division by zero. This also smooths the data to avoid unrelated variations.\notedme{what's an unrelated variation?}
